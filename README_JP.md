...
OpenTypeNo はローカル音声認識に [coli](https://github.com/marswaveai/coli) を使用します。`Node.js` と `ffmpeg` のインストールが必要です：
...
> **注意**: 初回使用時、アプリはバックグラウンドでローカル音声モデルを自動的にダウンロードします。ネットワークの速度によっては数分かかる場合があり、進行状況が表示されます。一度ダウンロードすれば、それ以降の文字起こしは瞬時に行われます！
